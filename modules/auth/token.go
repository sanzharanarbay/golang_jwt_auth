...
func (t *tokenservice) CreateToken(userId string) (*TokenDetails, error) {
	...
	accessTokenExpireTime, _ := strconv.Atoi(accessTokenExpire)
	refreshTokenExpire := os.Getenv("REFRESH_TOKEN_EXPIRE")
	refreshTokenExpireTime, _ := strconv.Atoi(refreshTokenExpire)


	td := &TokenDetails{}
	td.AtExpires = time.Now().Add(time.Minute * time.Duration(accessTokenExpireTime)).Unix() //expires after 30 min
	...
	td.RtExpires = time.Now().Add(time.Hour * 24 * time.Duration(refreshTokenExpireTime)).Unix() //expires after 1 week min
